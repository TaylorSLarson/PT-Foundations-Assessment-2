...
return 'Yes, we can deliver!'
    ...
    return 'Sorry, we cannot deliver.'
}
}
...
function canWeDeliverTwo(zipcodes){
   if(deliveryAreaZipCodes.includes(zipcodes) === true)
{
    return 'Yes, we can deliver!'
}else {
...
// call const deals and call title

// Object.keys(deals).forEach(prop => console.log(prop))


// console.log(deals)
deals.title.forEach(title => title.replace('10', '15'))
...
deals.desc.forEach(desc => desc.replace('March', 'April'))